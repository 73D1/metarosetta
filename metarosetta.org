...
The first root-level headline within the org configuration file. Tagged as ~:root:~.
...
Within a single org configuration file, there can be one or more expressions of this type, defined as root-level headlines following the /root/ expression. Tagged as ~:output:~.
...
(cl-defmethod mrosetta-org-parse ((oexpression mrosetta-org-expression) oelement)
  ...
  (let* ((match-id-key-string (org-ml-headline-get-node-property "ID" oelement))
...
#+NAME: mrosetta-org-serialize
#+BEGIN_SRC emacs-lisp :tangle yes
(cl-defmethod mrosetta-org-serialize ((oexpression mrosetta-org-expression))
  "Serialize the OEXPRESSION into an org-ml headline element. Also, recursively serialize all the contained matches."
  (let* ((mlexpression (slot-value oexpression 'mlexpression))
    ...
    (setq oelement (org-ml-headline-set-node-property "ID" match-id-key-string oelement))
