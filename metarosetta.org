...
**** Initializer
Defining proper context initialization.

#+NAME: mrosetta-context-org-initializer
#+BEGIN_SRC emacs-lisp :tangle yes
(cl-defmethod initialize-instance :after ((context mrosetta-context-org) &rest _slots)
  "Initialize the CONTEXT instance by loading the index datastore from file and compiling the Metalanguage expressions, as well as register for saving the index before killing Emacs."
  (let* ((file (slot-value context 'index-file)))
    ;; Load the index datastore
    (setf (slot-value context 'index) (or (when (file-exists-p file)
                                            (eieio-persistent-read file 'mrosetta-context-org-db))
    ...
    (setf (slot-value context 'mlexpressions)
          (mapcar (lambda (org-mlexpression-pair)
                  ...
                  (mrosetta-context-org-collection-items (mrosetta-context-org-db-mlexpressions (slot-value context 'index)))))
...
<<mrosetta-context-org-initializer>>
...
** Setup
Set up and initialize the Metarosetta org context.
...
(defvar mrosetta-context-org-default (mrosetta-context-org :index-file "~/org/mrosetta-index.db"
                                                           :connector (mrosetta-connector-coda :token token)))
#+END_SRC
...
(map! :map org-mode-map
      :n "s-RET" (lambda () (mrosetta-context-org-process-heading mrosetta-context-org-default)))
#+END_SRC
