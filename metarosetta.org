...
*** Connector Index
All registered connectors, indexed by its compatible type.
...
*** Configuration Index
The index of all the loaded configuration org files, corresponding to each respective /root/ metalanguage expression used to check against potential matches.
...
A global index of all the tracked matches, across all loaded metalanguage expressions, corresponding to their source configuration org file.

#+NAME: mrosetta-index-matches
#+BEGIN_SRC emacs-lisp :tangle yes
...
*** Original Match Source Reverse Index
For implicit synchronization to work, there needs to be a reverse index on all the source files containing any tracked matches.
...
***** Output Expression
Generate a new output match in context of the output expression object, based on the provided semantic data and the output's template.
...
****** Update from Original Match

****** Update from Output Match
...
**** Read Match
Read an output match from a specified target via the connector.
...
***** Read Match
Read a match from within a given section of the output file defined by a specific unique id.
...
***** Update Match
Update an output match within a given section of the output file defined by a specific unique id.
