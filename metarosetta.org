...
This section covers all the connector-specific extensions to the metalanguage expression org entry.

These enable persistence of properties particular to a given connector.

******* Coda Extensions
Definition of Coda-specific slots required to synchronize matches to corresponding Coda documents and tables within.

#+NAME: mrosetta-context-org-mlexpression-coda
#+BEGIN_SRC emacs-lisp :tangle yes
...
#+END_SRC
**** Persistent Context
A context object used as an entry point for the indexed data, as well as a point of serialization to and deserialization from the disk.
